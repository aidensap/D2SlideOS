...
def _add_chart_slide_from_image(prs, image_path, report_label):
    slide = prs.slides.add_slide(prs.slide_layouts[6])
    slide.background.fill.solid()
    slide.background.fill.fore_color.rgb = DARK_BG
    ...
    slide.shapes.add_picture(image_path, Inches(0.5), Inches(1.0), Inches(9), Inches(5.5))
...
def generate_slides_from_screenshot(screenshot_path: str, report_label: str, insights: str, lang: str = "en") -> str:
    return generate_slides_from_screenshots([screenshot_path], report_label, [insights], lang)


def generate_slides_from_screenshots(screenshot_paths: list, report_label: str, insights_list: list, lang: str = "en") -> str:
    prs = Presentation()
    prs.slide_width = Inches(10)
    prs.slide_height = Inches(7)

    first_insights = insights_list[0] if insights_list else ""
    first_line = next((l for l in first_insights.split("\n") if l.strip()), "")
    _add_title_slide(prs, report_label, first_line, lang)

    slide_title = "AI 数据洞察" if lang == "zh" else "AI Data Insights"

    for i, (screenshot_path, insights) in enumerate(zip(screenshot_paths, insights_list), 1):
        page_label = f"{report_label} — P{i}" if len(screenshot_paths) > 1 else report_label
        _add_screenshot_slide(prs, screenshot_path, page_label, lang)
        chunks = _split_insights(insights)
        for j, chunk in enumerate(chunks, 1):
            _add_insights_slide(prs, chunk, slide_title, page=j, total=len(chunks))
...
def generate_slides(df, report_label, insights, lang="en", chart_prompt=""):
    prs = Presentation()
    prs.slide_width = Inches(10)
    prs.slide_height = Inches(7)
